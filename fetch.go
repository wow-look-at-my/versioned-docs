...
func listSemverVersions(remote string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fetch-aggregate: listing versions of %s failed: %w\nhint: if the repository is private, pass a -remote URL that embeds a credential able to read it (locally, point -remote at a local clone instead)", sanitizeCredentials(remote), err)
	}
...
func extractAggregateTree(opts fetchOptions) ([]string, int, error) {
	...
	if !fetched {
		return nil, 0, fmt.Errorf("fetch-aggregate: fetching branch %q from %s failed: %w\nhint: if the repository is private, pass a -remote URL that embeds a credential able to read it (locally, run `git fetch origin %s` in the clone first)", opts.Branch, sanitizeCredentials(opts.Remote), fetchErr, opts.Branch)
	}
